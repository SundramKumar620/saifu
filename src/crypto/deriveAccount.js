...
export function deriveAccountLocally(mnemonic, accountIndex) {
    ...
    // 1️⃣ mnemonic → seed (512-bit)
    const seed = mnemonicToSeedSync(mnemonic);

    // 2️⃣ create HD root (ed25519 SLIP-0010)
    const hd = HDKey.fromMasterSeed(seed);

    // 3️⃣ Solana derivation path (hardened)
...
export function derivePrivateKey(mnemonic, accountIndex) {
    ...
    // 6️⃣ Return private key as base58 string (Solana standard format)
